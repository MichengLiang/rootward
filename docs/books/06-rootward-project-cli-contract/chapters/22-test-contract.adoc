...
测试应证明负路径稳定。`PROJECT_NOT_FOUND`、`PROJECT_CONFIG_NOT_FOUND`、`CONFIG_PARSE_FAILED`、`CONFIG_INVALID`、`SOURCE_NOT_FOUND`、`SOURCE_ROOT_NOT_FOUND`、`SCANNER_NOT_REGISTERED`、`DISCOVERY_FAILED` 或 `SCAN_FAILED` 的触发条件一旦进入契约，就必须有对应断言。没有测试的错误码会退化为文档承诺。
