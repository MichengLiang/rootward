...
* TypeScript 生成物 package.json 的 `name` 和 `bin` 与身份一致。
...
* Rust 模板源通过 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test` 和 `cargo build`。
* 生成后的 Rust 项目通过 `cargo fmt --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test` 和 `cargo build`。
