...
Rootward 模板是业务工具的起点。Creator 实例化模板时，必须替换工具身份、生成物包元数据和文档表面，同时保留 Rootward 基础契约。手工复制不是 Rootward 默认消费路径。
...
生成后的文档也属于公共契约。README、help text、错误 message、默认配置注释和示例命令必须使用替换后的工具身份。生成物不得残留 `__ROOTWARD_` identity token。残留 token 表示 Creator 替换失败，必须返回 `TOKEN_RESIDUE_FOUND`。
